...
public class CameraPreview extends ViewGroup implements SurfaceHolder.Callback, Camera.AutoFocusCallback {
	...
	public static int PHOTO_SIZE_W = 2000;
	public static int PHOTO_SIZE_H = 2000;
	...
	public void initCamera() {
		if (mCamera != null) {
			...
			if (adapterSize != null) {
				...
				Log.e("bestPic", adapterSize.width+"  "+adapterSize.height);
			}
			if (mPreviewSize != null) {
				params.setPreviewSize(mPreviewSize.width, mPreviewSize.height);
				Log.e("bestPre", mPreviewSize.width+"  "+mPreviewSize.height);
				mSurfaceView.setLayoutParams(new FrameLayout.LayoutParams(screenWidth, mPreviewSize.width*screenWidth/mPreviewSize.height));
			}
...
//			if(CameraActivity.mCurrentCameraId == 1){
//				setCameraDisplayOrientation((Activity) mContext, CameraActivity.mCurrentCameraId, mCamera);
				List<Size> previewSizes = params.getSupportedPreviewSizes();
				int lengths = previewSizes.size();
				Size s = previewSizes.get(0);
				for (int i = 0; i < lengths; i++) {
					Size size = previewSizes.get(i);
					Log.e("Supportedpreview : ", previewSizes.get(i).width + "x" + previewSizes.get(i).height);
					if((size.width >= screenWidth) && equalRate(size, (float)mPreviewSize.width/(float)mPreviewSize.height)){
						s = size;
						break;
					}
				}
				...
				s = pictureSizes.get(0);
				for (int i = 0; i < length; i++) {
					Log.e("Supported : ", pictureSizes.get(i).width + "x" + pictureSizes.get(i).height);
					Size size = pictureSizes.get(i);
					if((size.width >= screenWidth) && equalRate(size, (float)adapterSize.width/(float)adapterSize.height)){
						s = size;
						break;
					}
				}
				PHOTO_SIZE_H = s.height/2;
				PHOTO_SIZE_W = s.width/2;
